...
def get_current_question() -> str:
    ...
    if question_paragraph:
        question = question_paragraph.get_text().strip().replace("Question:", "").strip()
...
def login_with_answer(answer: str) -> str:
    ...
    login_response = requests.post(WEBSITE_URL, data=form_data, timeout=TIMEOUT_DEFAULT)
    if login_response.status_code != HTTPStatus.OK:
        LOG.error("Login failed, status code = %s", login_response.status_code)
    return login_response.text

def process_response(login_response: str) -> str:
    """Extract flags and secret link from login response"""
    flag_pattern = re.compile(r"{{FLG:[a-zA-Z0-9]+}}")
    flags = flag_pattern.findall(login_response)
    if flags:
        LOG.info("Flags found: %s", " ".join(flags))
    soup = BeautifulSoup(login_response, "html.parser")
    LOG.debug("The following response was sent after login: %s", login_response)
...
if __name__ == "__main__":
    ...
    response = login_with_answer(current_answer)
    link, flags = process_response(response)
